    ...
    while True:
        ...
        for i in range(0, len(scan_times)-1): 
            if scan_times[i+1] > currenttime > (scan_times[i] + timedelta(seconds =90)): # check whether current time falls within 90s of the start of any scan and the end of that same scan
                ...
                np.savetxt(cwd + '/' + str(scan_times[current_scan]) + '.txt', spec_array, header='Scan start time: ' + scan_times[i] + ' jd', delimiter=",")
                ...
                #        print("Averaged spectrum for scan " + str(scan_times[current_scan]) + " written to file.")
                spec_list = []
            else:
                continue
